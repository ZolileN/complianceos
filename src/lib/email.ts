...
const FROM_EMAIL = 'ComplianceOS <no-reply@resend.dev>';
    ...
    const data = await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
          ...
          <p>We received a request to reset the password for your ComplianceOS account.</p>
          ...
          <p style="font-size: 0.8em; color: #999;">&copy; ${new Date().getFullYear()} ComplianceOS. All rights reserved.</p>
        </div>
      `,
    });
...
export async function sendTeamInviteEmail(email: string, name: string, role: string, inviteUrl: string) {
  try {
    const data = await resend.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject: 'You have been invited to join ComplianceOS',
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
          <h2 style="color: #0F172A;">Welcome to ComplianceOS!</h2>
          <p>Hello ${name},</p>
          <p>You have been invited to join your firm's workspace on ComplianceOS as a <strong>${role.replace('_', ' ')}</strong>.</p>
